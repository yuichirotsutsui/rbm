...
class GBRBM(object):
	...
	def contrastive_divergence(self, lr=0.1, k=1, input=None, batch_size = 10):
		...
		while 1:
			...
			i += 1
			self.W += lr * (cupy.dot(input_data.T, ph_mean)- cupy.dot(nv_means.T, nh_means))
	...
	def propdown(self, h):
		pre_sigmoid_activation = cupy.dot(h, self.W.T) + self.vbias
		return pre_sigmoid_activation

	def gibbs_hvh(self, h0_sample):
		v1_mean, v1_sample = self.sample_v_given_h(h0_sample, var = 1)
		h1_mean, h1_sample = self.sample_h_given_v(v1_sample)
		return [v1_mean, v1_sample, h1_mean, h1_sample]

	def make_memory(self, v0_sample, num, data = None, var = 1, simple_sample = False):
	...
	def get_reconstruction_cross_entropy(self):
		...
		v1_sample = self.normal(size = v1_mean.shape, p =  v1_mean, var = 0.00000000001)
		cross_entropy = cupy.mean((self.input - v1_sample) * (self.input - v1_sample))
		return cross_entropy
	def reconstruct(self, v):
		h = sigmoid(cupy.dot(v, self.W) + self.hbias)
		reconstructed_v = sigmoid(cupy.dot(h, self.W.T) + self.vbias)
